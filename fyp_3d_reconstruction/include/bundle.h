...
void toEigen( const cv::Mat R[], const cv::Mat T[], const cv::Mat theta[], const std::vector<cv::Point3f> pts3, Eigen::Matrix3f Rot[], Eigen::Vector3f Theta[], Eigen::Vector3f Tra[], Eigen::Vector3f P3[], int m, int n);

// convert variables to OpenCV library
void toCV( cv::Mat R[], cv::Mat T[], cv::Mat theta[], std::vector<cv::Point3f>& pts3, const Eigen::Matrix3f Rot[], const Eigen::Vector3f Theta[], const Eigen::Vector3f Tra[], const Eigen::Vector3f P3[], int m, int n);

// get reprojections in image frame
std::vector< std::vector<cv::Point2f> >  getReprojection( const Eigen::Vector3f Theta[], const Eigen::Vector3f Tra[], const Eigen::Vector3f P3[], int m, int n);

// get observations in image frame
std::vector< std::vector<cv::Point2f> >  getObservation( const std::vector< std::vector<cv::Point2f> >& z, int m, int n);

// update error vector
void updateError( const std::vector< std::vector<cv::Point2f> >& z, const std::vector< std::vector<cv::Point2f> >& rep, const std::vector< std::vector<cv::Point2f> >& z_img, const int m_cam, const int n_pts, Eigen::VectorXf& err);
