 ...
 */
#include "viz.h"
 void viz(const cv::Mat points4D,ros::Publisher& pub_pts,ros::Publisher& pub_cam,cv::Mat t[],Eigen::Quaterniond q[]){
...
// $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
        ...
        cam_2.pose.position.x = t[3].at<double>(0);
        cam_2.pose.position.y = t[3].at<double>(1);
        cam_2.pose.position.z = t[3].at<double>(2);

        cam_1.pose.orientation.x = q0.x();
        cam_1.pose.orientation.y = q0.y();
        cam_1.pose.orientation.z = q0.z();
        cam_1.pose.orientation.w = q0.w();
        cam_2.pose.orientation.x = q[3].x();
        cam_2.pose.orientation.y = q[3].y();
        cam_2.pose.orientation.z = q[3].z();
        cam_2.pose.orientation.w = q[3].w();
        ...
        for (int i=0;i<n_matches[0];i++)
        {
            ...
            p.x = points4D.at<double>(i,0);
            p.y = points4D.at<double>(i,1);
            p.z = points4D.at<double>(i,2);
